...
async def asci(event):
    ...
    if reply_message.sender.bot:
        await event.edit("Bir istifadəçi qeyd edin.")
        return
    asc = await event.edit("**ASCII** `...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("@asciiart_bot'u `blokdan çıxardın və bir daha yenidən yoxlayın`")
            return
        if response.text.startswith("Forward"):
            await event.edit(
                "Gizlilik ayarlarınızı düzəldin və yenidən yoxlayın."
            )
        else:
            await event.delete()
            await event.client.send_file(
                ...
                caption=f"@SpaceUserBot ilə ascii uyğulandı",
            )
